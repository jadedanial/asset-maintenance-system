...
import { Card, Select, Col, Row, Button, Typography, List, Tag, notification } from 'antd';
      ...
      schedules.map((schedule) => (schedule.id === id ?
        <>
          <Row style={{paddingTop: "20px", flexWrap: "wrap"}}>
            ...
            renderItem={(item) => (
              <List.Item>
                <Card title={<Title><p className="medium-card-title">{item.title}</p></Title>} {...cardlayout} style={{margin: "10px"}} className="ant-card-custom-hover"><Tag color="blue" style={{width: "100%"}}><p className="small-font" style={{padding: "10px"}}>{item.description}</p></Tag></Card>
              </List.Item>
            )} />
          </Row>
        </>:<></>
        ...
        <Card size="small" style={{width: "100%"}}>
          <Row>
            ...
            <Col span={19} style={{margin: "10px 10px 10px 2px", marginLeft: "3px"}}>
              ...
              value={schedname} options={schedules.map((schedule) => {return {value: schedule.id, label: schedule.sched_name};})} onChange={onChange} />
            </Col>
          </Row>
        </Card>
      </Row>
      <Row style={{marginTop: "20px"}}>
        {scheduleSwitch(schedid)}
      </Row>
    </>
  );
